...
def get_code_metrics(commit_hash, commit_data, repo_path):
    ...
    for graph in graphs_new:
        name = get_filename_for_graph("Exports", commit_data["filename_new"], commit_hash, graph, "new")
        visualize_call_graph(graph, name)
    # graphs_old = generate_ast_graph(commit_data["source_old"])
    # for graph in graphs_old:
    #     name = get_filename_for_graph("Exports", commit_data["filename_old"], commit_hash, graph, "old")
    #     visualize_call_graph(graph, name)
