...
from build.pydriller import get_and_insert_commits_data, get_and_store_commits_data, get_pydriller_metric
...
def main():
    ...
    for commit_hash, commit_data in commits.items():
        ...
        print(f"COMMIT COUNT: The commit count for this file changed from {get_pydriller_metric(repo_path, "34b172cf71ad46ccbcc80a6983ca692f7e6bb469", commit_hash, commit_data["filename_new"], "commit_count") - 1} to {get_pydriller_metric(repo_path, "34b172cf71ad46ccbcc80a6983ca692f7e6bb469", commit_hash, commit_data["filename_new"], "commit_count")}")
        print(f"CODE CHURN: The code churn changes from {get_pydriller_metric(repo_path, "34b172cf71ad46ccbcc80a6983ca692f7e6bb469", commit_hash, commit_data["filename_new"], "code_churn") - (commit_data["additions"] - commit_data["deletions"])} to {get_pydriller_metric(repo_path, "34b172cf71ad46ccbcc80a6983ca692f7e6bb469", commit_hash, commit_data["filename_new"], "code_churn")}")
        print(f"DOCUMENTATION RATIO: The documentation ratio changed from {analyse_source_code(commit_data["source_old"], 'docuementation_LOC')[6].split(":")[1]} to {analyse_source_code(commit_data["source_new"], 'docuementation_LOC')[6].split(":")[1]}")
